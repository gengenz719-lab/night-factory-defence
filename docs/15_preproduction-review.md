...
| D-11 | Web版の位置づけ | Web書き出しはソロデモ配布専用。マルチプレイはWindowsビルドのみ | 2026-07-11決定 |
...
| 日付 | 変更 | 理由 | 影響範囲 |
|---|---|---|---|
| 2026-07-11 | D-11追加(Web=ソロデモ専用、マルチ=Windowsビルド) | Godot 4のENetはWebエクスポートで動作しない(ブラウザは生UDP不可)。WebRTC対応はコストが大きく初期スコープ外 | 配布計画のみ。ゲームルールへの影響なし |
| 2026-07-11 | 14 §7に送信方向の明記、§8に通信テスト記録指標を追加 | Spike B(2人ENet通信)合格。検証で得た運用知見の反映 | 14 §7〜8。ルール変更なし |
