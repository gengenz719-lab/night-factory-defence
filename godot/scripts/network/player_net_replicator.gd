...
func _capture_local_input() -> void:
	...
	if NetworkSession.is_host_authority():
		_apply_host_input(NetworkSession.local_peer_id(), input_sequence, axis, aim, wants_fire, wants_interact, wants_dodge)
	else:
		...
		_submit_input.rpc_id(NetworkSession.HOST_PEER_ID, input_sequence, axis, aim, wants_fire, wants_interact, wants_dodge)


@rpc("any_peer", "call_remote", "unreliable_ordered", 1)
func _submit_input(sequence: int, axis: Vector2, aim: Vector2, wants_fire: bool, wants_interact: bool, wants_dodge: bool) -> void:
	...
	_apply_host_input(sender_id, sequence, axis, aim, wants_fire, wants_interact, wants_dodge)


func _apply_host_input(peer_id: int, sequence: int, axis: Vector2, aim: Vector2, wants_fire: bool, wants_interact: bool, wants_dodge: bool) -> void:
...
func _receive_player_snapshot(peer_id: int, ack_sequence: int, authoritative_position: Vector2, aim: Vector2, hp: float, downed: bool, departed: bool, invulnerability: float, dodge_cooldown_value: float, down_time: float, return_time_value: float, revive_progress_value: float) -> void:
	...
	if peer_id == NetworkSession.local_peer_id():
		...
		player.apply_network_snapshot(authoritative_position, aim, hp, downed, departed, invulnerability, dodge_cooldown_value, down_time, return_time_value, revive_progress_value)
...
func _receive_shot_event(shooter_peer_id: int, origin: Vector2, aim: Vector2) -> void:
	confirmed_shots += 1
	if shooter_peer_id != NetworkSession.local_peer_id(): coordinator.spawn_cosmetic_projectile_at(origin, aim)
