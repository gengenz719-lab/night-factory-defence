...
func update_status(wave: int, state_text: String, time_left: float, player: CrewPlayer, vehicle: SurvivalVehicle, kills: int, relics: Array[String]) -> void:
	var hp_text: String = "DOWN" if player.is_downed else "%d / %d" % [roundi(player.hp), roundi(player.max_hp)]
	player_label.text = "P1  ガンナー\nHP  %s\n撃破  %d" % [hp_text, kills]
	wave_label.text = "WAVE %d / 2\n%s   %02d:%02d" % [wave, state_text, floori(time_left) / 60, floori(time_left) % 60]
	...
	message_label.text = "取得: %s" % " / ".join(relics) if not relics.is_empty() else "車両を守りながら90秒耐えろ"


func show_relic_choices() -> void:
	...
	var cards: Array[Dictionary] = [
		{"name": "増し弾", "tag": "戦闘", "desc": "プレイヤーの射撃ダメージ +25%", "color": Color("#b94e45")},
		{"name": "補強板", "tag": "車両", "desc": "車体最大HP +200\n外装最大HP +20%", "color": Color("#3b8290")},
		{"name": "高速修理", "tag": "特殊", "desc": "修理速度 +50%\n戦闘中の立て直しを強化", "color": Color("#76538d")},
	]
	for index: int in range(cards.size()):
		var card: Dictionary = cards[index]
		var button := Button.new()
		button.custom_minimum_size = Vector2(350, 300)
		button.text = "%d\n[%s]\n\n%s\n\n%s" % [index + 1, card.tag, card.name, card.desc]
		button.add_theme_font_size_override(&"font_size", 22)
		var normal := StyleBoxFlat.new()
		normal.bg_color = Color(card.color, 0.78)
...
func show_end(victory: bool, kills: int, relics: Array[String]) -> void:
	overlay.visible = true
	...
	summary.text = "2 Wave テスト完了\n撃破数: %d\nレリック: %s" % [kills, " / ".join(relics) if not relics.is_empty() else "なし"] if victory else "車両が破壊された\n撃破数: %d" % kills
